...
def generate_questions(jd_text, resume_text):
   prompt = (
       ...
       f"To ensure personalisation, ask probing question in this style: As mentioned in your resume,"
       f"Include recommended responses.\n\n"
       f"Job Description:\n{jd_text}\n\n"
       f"Resume:\n{resume_text}"
   )
   response = model.generate_content(prompt)
   st.session_state.questionsTokenCount += response.usage_metadata.total_token_count
   st.text("Model Response:")
   st.write(response.text)  # Show the full response from the model
   st.write(f"Token consumption:{st.session_state.questionsTokenCount}")
