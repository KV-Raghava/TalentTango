...
load_dotenv()
modelCount = GenerativeModel('models/gemini-1.0-pro-latest')
...
def generate_initial_jd(role, min_exp, max_exp, requirements):
    ...
    st.session_state.jdTokenCount += response.usage_metadata.total_token_count
...
# Initialize the Gemini Pro model and start chat session
    ...
    if st.session_state.jd or st.session_state.updatedJd:
        ...
        st.write(f"Token consumption: {st.session_state.jdTokenCount}")
